...
Write-Information "Switching network connection to Private for WinRM"
...
if ('${winrm_initialization}' -eq 'true') {
    ...
    try {
        Write-Information "Delete any existing WinRM listeners"
    ...
    Write-Information "Create a new WinRM listener and configure"
    ...
    Write-Information "Configure UAC to allow privilege elevation in remote shells"
    $Key = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System'
    $Setting = 'LocalAccountTokenFilterPolicy'
    Set-ItemProperty -Path $Key -Name $Setting -Value 1 -Force

    Write-Information "turn off PowerShell execution policy restrictions"
    Set-ExecutionPolicy -ExecutionPolicy Unrestricted

    Write-Information "Configure and restart the WinRM Service; Enable the required firewall exception"
